...
func (c *Client) BlockCustomer(userID string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	req := &dbpb.BlockCustomerRequest{
		UserId: userID,
	}

	_, err := c.DB.BlockCustomer(ctx, req)
	return err
}

func (c *Client) GetAllBlockedCustomers() ([]string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	req := &dbpb.GetAllBlockedCustomersRequest{}

	resp, err := c.DB.GetAllBlockedCustomers(ctx, req)
	if err != nil {
		return nil, err
	}
